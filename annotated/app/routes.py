...
def disk_io_processes():  # def disk_io_processes():
    ...
    try:  # try:
        ...
        sort_by = request.args.get('sort', 'total')  # sort_by = request.args.get('sort', 'total')
        
        if sort_by == 'read':  # if sort_by == 'read':
            processes.sort(key=lambda x: x['read_bytes'], reverse=True)  # processes.sort(key=lambda x: x['read_bytes'], reverse=True)
        elif sort_by == 'write':  # elif sort_by == 'write':
            processes.sort(key=lambda x: x['write_bytes'], reverse=True)  # processes.sort(key=lambda x: x['write_bytes'], reverse=True)
        else:  # else:
...
def get_processes():  # def get_processes():
    ...
    try:  # try:
        ...
        # Sort processes  # # Sort processes
        sort_by = request.args.get('sort', 'cpu')  # sort_by = request.args.get('sort', 'cpu')
        
        if sort_by == 'memory':  # if sort_by == 'memory':
            processes.sort(key=lambda x: x['memory_percent'], reverse=True)  # processes.sort(key=lambda x: x['memory_percent'], reverse=True)
        else:  # else:
